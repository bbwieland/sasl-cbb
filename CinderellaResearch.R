...
teams_join_notP6_good = team_data_lead %>%
  filter(!(conf %in% c("B10","B12","ACC","BE","SEC","P12", "P10"))) %>%
  filter(w_percent > 0.5)

teams_join_notP6_bad = team_data_lead %>%
  filter(!(conf %in% c("B10","B12","ACC","BE","SEC","P12", "P10"))) %>%
  filter(w_percent <= 0.5)

teams_join_notP6_good
teams_join_notP6_bad
colMeans(teams_join_notP6_good[,"avg_rating"])
colMeans(teams_join_notP6_bad[,"avg_rating"])
